...
onMounted(() => casinosStore.fetchCasinos({ per_page: 100 }))
...
async function create(): Promise<void> {
  ...
  loading.value = true
  try {
    const res = await offersApi.createSpecialOffer({ ...form, casino_id: form.casino_id as number })
    store.upsert(res.data)
    toast.add({ severity: 'success', summary: 'Created', detail: 'Special offer created.', life: 3000 })
    router.push({ name: 'special-offers-edit', params: { id: res.data.id } })
  } catch (e: unknown) {
    if (axios.isAxiosError(e) && e.response?.status === 422) {
      ...
      formError.value = 'Failed to create special offer.'
    }
  } finally {
    loading.value = false
  }
